...
const fetchMembersSlice = createSlice({
    ...
    reducers: {
        ...
        multipleRecorDelete : (state) => {

            const handleDeletion = (membersData) => {
                return !state.selectedCheckbox.includes(membersData.id)
            }

            const newMembersData = state.membersData.filter(handleDeletion)

            state = { ...state, membersData: newMembersData , selectedCheckbox : [] }
            return state
